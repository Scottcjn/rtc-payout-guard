...
def _get(url: str, tok: str, target_hint: tuple = ("",)) -> list:
    ...
    while page <= 20:
        ...
        for attempt in range(3):
            r = subprocess.run(
                ["curl", "-s", "--max-time", "60", "-H", f"Authorization: token {tok}",
                 "-H", "Accept: application/vnd.github+json", f"{url}?per_page=100&page={page}"],
        ...
        if isinstance(data, dict):
            ...
            if data.get("message") == "Not Found" and _repo_exists(target_hint[0], tok):
                return []
...
def make_fetch(tok: str):
    def fetch(kind: str, target: str) -> list[str]:
        if kind == sw.STAR:
            return [u["login"] for u in _get(f"https://api.github.com/repos/{target}/stargazers", tok, (target,))]
        if kind == sw.FORK:
            return [f["owner"]["login"] for f in _get(f"https://api.github.com/repos/{target}/forks", tok, (target,))]
        if kind == sw.FOLLOW:
            return [u["login"] for u in _get(f"https://api.github.com/users/{target}/followers", tok)]
